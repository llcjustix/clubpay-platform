...
const CONTROLLER_RELEASE_URL = 'https://github.com/llcjustix/clubpay-platform/releases/download/controller-v0.2.9/ClubPay-Controller-win-x64.zip';
const MANAGER_RELEASE_URL = 'https://github.com/llcjustix/clubpay-core-agent/releases/download/v0.4.15/ClubPay-Manager-Desktop-win-x64.zip';
const AGENT_RELEASE_URL = 'https://github.com/llcjustix/clubpay-core-agent/releases/download/v0.4.15/ClubPay-Agent-win-x64.zip';
...
function windowsInstallerBootstrap(spec: WindowsInstallerBootstrap) {
  ...
  const taskCleanup = spec.taskName
    ? `Get-ScheduledTask -TaskName '${spec.taskName}' -ErrorAction SilentlyContinue | Unregister-ScheduledTask -Confirm:$false`
    : '';

  return `# ClubPay ${spec.title} — generated by ClubPay Web Admin. Do not share this file.
...
function SettingsPage({ auth, selectedClubID, currentPath, onClubChange, onLogout, onReloadAuth }: WorkspaceProps & { onReloadAuth: () => void | Promise<void> }) {
  ...
  function downloadControllerEnrollment() {
    ...
    downloadTextFile(
      isManager ? 'ClubPay-Manager-setup.ps1' : 'ClubPay-Controller-setup.ps1',
    ...
    setMessage('Скачан один файл установки. На нужной VM кликните по нему правой кнопкой → «Run with PowerShell», затем подтвердите окно Windows.');
  }
  ...
  async function downloadAgentEnrollment(pc: ManagedPC) {
    ...
    try {
      ...
      downloadTextFile(
        `ClubPay-Agent-${pc.external_pc_id || pc.number}-setup.ps1`,
        windowsInstallerBootstrap({
      ...
      setMessage(`Скачан один файл установки для «${pc.label}». Перенесите только его на этот игровой ПК и запустите через «Run with PowerShell».`);
    } catch (err) {
      setError(String((err as Error).message || err));
    }
  }
    ...
    <main className="shell workspace-shell">
      ...
      {!settings || !clubForm ? (
        ...
        <section className="settings-grid">
          ...
          {!creatingClub && (
            <Panel className="stack settings-wide">
              <SectionTitle icon={<KeyRound size={18} />} title="Локальный Controller" caption="Кабинет выдаёт один установочный файл: он сам скачает сборку и выполнит привязку. Секрет действует 30 минут и исчезает после первого использования." />
