...
type WindowsInstallerBootstrap = {
  title: string;
  releaseURL: string;
  ...
  processNames: string[];
  taskName?: string;
  removeBeforeInstall?: string[];
};

// A generated bootstrap is deliberately self-contained: an operator downloads
// one file from the authenticated cabinet, accepts UAC, and never has to handle
// an enrollment secret or a second archive manually.
function windowsInstallerBootstrap(spec: WindowsInstallerBootstrap) {
  const enrollment = base64UTF8(JSON.stringify(spec.enrollment, null, 2));
  const processNames = spec.processNames.map((name) => `'${name}'`).join(', ');
  const removePaths = [...(spec.removeBeforeInstall || []), spec.installDirectory].map((path) => `'${path}'`).join(', ');
  const taskCleanup = spec.taskName
    ? `# End a running task before unregistering it. Unregister alone leaves its
# Controller/PostgreSQL child process alive long enough to lock C:\\ClubPay.
& cmd.exe /c 'schtasks.exe /End /TN "${spec.taskName}" >nul 2>nul'
& cmd.exe /c 'schtasks.exe /Delete /TN "${spec.taskName}" /F >nul 2>nul'`
    : '';
  const freshInstall = `${taskCleanup}
$clubPayProcessNames = @(${processNames})
# The Controller owns a PostgreSQL child process. Stop the task first, then
# retry process termination because PostgreSQL shuts down asynchronously.
for ($attempt = 1; $attempt -le 4; $attempt++) {
  Get-CimInstance Win32_Process | Where-Object {
    ...
    # A process can exit between discovery and Terminate; that is success.
...
foreach ($path in @(${removePaths})) {
  for ($attempt = 1; $attempt -le 5 -and (Test-Path $path); $attempt++) {
  ...
  const updateBranch = spec.updaterName
    ...
    $updater = Get-ChildItem -Path $stage -Filter '${spec.updaterName}' -File -Recurse | Select-Object -First 1
    if ($null -eq $updater) { throw 'The ClubPay package does not contain ${spec.updaterName}.' }
    Write-Host 'Updating the configured ClubPay ${spec.title}…' -ForegroundColor Cyan
    & $updater.FullName
    if ($LASTEXITCODE -ne 0) { throw 'The ClubPay updater did not finish successfully.' }
    Write-Host 'ClubPay ${spec.title} is updated.' -ForegroundColor Green
  } else {`
    : '';
  const updateOnlyFailure = `throw 'Configured ClubPay ${spec.title} was not found. Use the first-install file from ClubPay Web Admin.'`;

  const powershell = `# ClubPay ${spec.title} — generated by ClubPay Web Admin. Do not share this file.
$ErrorActionPreference = 'Stop'
Write-Host 'Preparing ClubPay ${spec.title}…' -ForegroundColor Cyan

$stage = Join-Path $env:TEMP ('clubpay-setup-' + [guid]::NewGuid().ToString())
$zip = Join-Path $env:TEMP ('clubpay-package-' + [guid]::NewGuid().ToString() + '.zip')
New-Item -ItemType Directory -Path $stage -Force | Out-Null
try {
  Invoke-WebRequest -Uri '${spec.releaseURL}' -OutFile $zip -UseBasicParsing
  Expand-Archive -Path $zip -DestinationPath $stage -Force
  ${updateBranch}
  ${spec.updateOnly ? updateOnlyFailure : freshInstall}
  $installer = Get-ChildItem -Path $stage -Filter '${spec.installerName}' -File -Recurse | Select-Object -First 1
  if ($null -eq $installer) { throw 'The ClubPay package does not contain ${spec.installerName}.' }
  New-Item -ItemType Directory -Path '${spec.installDirectory}' -Force | Out-Null
  Copy-Item -Path (Join-Path $installer.Directory.FullName '*') -Destination '${spec.installDirectory}' -Recurse -Force
  $enrollment = [Text.Encoding]::UTF8.GetString([Convert]::FromBase64String('${enrollment}'))
  [IO.File]::WriteAllText((Join-Path '${spec.installDirectory}' '${spec.enrollmentFilename}'), $enrollment, [Text.UTF8Encoding]::new($false))
  & (Join-Path '${spec.installDirectory}' '${spec.installerName}')
  if ($LASTEXITCODE -ne 0) { throw 'The ClubPay installer did not finish successfully.' }
  Write-Host 'ClubPay ${spec.title} is installed.' -ForegroundColor Green
  ${spec.updaterName ? '}' : ''}
}
finally {
...
function LauncherCatalogPage({ auth, selectedClubID, currentPath, onClubChange, onLogout }: WorkspaceProps) {
  ...
  return <main className="shell workspace-shell">
    ...
    <Panel className="stack">
      ...
      {loading ? <EmptyState text="Загружаем каталог" /> : visible.length === 0 ? <EmptyState text="Агент пока не прислал приложения. Откройте лаунчер на нужном ПК или дождитесь ближайшей синхронизации." /> : <div className="table-wrap"><table><thead><tr><th>Приложение</th><th>Компьютер</th><th>Категория</th><th>Последняя синхронизация</th></tr></thead><tbody>{visible.map((app) => <tr key={app.id}><td><strong>{app.name}</strong></td><td>{app.pc_label}</td><td><select value={app.category} onChange={(event) => setCategory(app, event.target.value as LauncherCatalogApp['category'])}><option value="shooter">Шутеры</option><option value="strategy">Стратегии</option><option value="other">Другое</option></select></td><td>{formatDateTime(app.last_seen_at)}</td></tr>)}</tbody></table></div>}
    </Panel>
  </main>;
}
  ...
  async function downloadControllerInstaller(nodeMode: 'edge' | 'manager') {
    ...
    try {
      ...
      windowsInstallerBootstrap({
        ...
        installerName: isManager ? 'install-manager.cmd' : 'install-windows.cmd',
