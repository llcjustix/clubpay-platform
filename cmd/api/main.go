...
func main() {
	...
	reconfigure := flag.Bool("reconfigure", false, "replace this Controller Node's Cloud enrollment while preserving its local data")
	...
	flag.Parse()
	if (*install && *uninstall) || (*setup && (*install || *uninstall)) || (*reconfigure && !*setup) {
	...
	if *setup {
		if err := setupControllerNode(*configPath, *activationCode, *activationURL, *nodeName, *reconfigure); err != nil {
			log.Fatal(err)
		}
		return
	}
...
func setupControllerNode(configPath, activationCode, activationURL, nodeName string, reconfigure bool) error {
	...
	if _, err := os.Stat(absoluteConfigPath); err == nil {
		if !reconfigure {
			return fmt.Errorf("%s already exists; this Controller is already configured (use --setup --reconfigure only to replace its Cloud enrollment)", absoluteConfigPath)
		}
		existing, err = envfile.Read(absoluteConfigPath)
		if err != nil {
			return fmt.Errorf("read existing controller config: %w", err)
		}
	...
	frontendBaseURL := strings.TrimRight(strings.TrimSpace(enrollment.FrontendBaseURL), "/")
	if frontendBaseURL == "" {
		return errors.New("activation response is missing frontend_base_url")
	}
	if len(existing) == 0 {
		existing = map[string]string{
			"APP_ENV": "production", "HTTP_ADDR": ":8080", "WEB_DIR": "web", "MIGRATIONS_DIR": "migrations",
			"LOCAL_DATABASE_MODE": "embedded", "LOCAL_DATABASE_DATA_DIR": "data/postgres", "LOCAL_DATABASE_RUNTIME_DIR": "runtime/postgres",
			"DATABASE_URL": "postgres://clubpay:" + randomLocalHex(24) + "@127.0.0.1:5432/clubpay?sslmode=disable",
			"CORE_MODE":    "ws", "CORE_TIMEOUT_MS": "10000", "WOL_ENABLED": "true", "WOL_BROADCAST_ADDR": "255.255.255.255:9", "WOL_WAIT_SECONDS": "60",
			"DEFAULT_PAYMENT_PROVIDER": "click", "MOCK_PAYMENTS_ENABLED": "false", "TELEGRAM_MINI_APP_ENABLED": "false", "TELEGRAM_POLLING_ENABLED": "false",
			"MANAGER_ONLINE_PAYMENTS_ENABLED": "false", "AUTO_UPDATE_ENABLED": "true", "AUTO_UPDATE_CHECK_SECONDS": "300", "AUTO_UPDATE_RING": "canary",
			"AUTO_UPDATE_CANARY_PC_IDS": "", "AUTO_UPDATE_CANARY_NODE_IDS": "", "AUTO_UPDATE_PILOT_CLUB_IDS": "", "AUTO_UPDATE_SELECTED_CLUB_IDS": "",
			"EDGE_SYNC_INTERVAL_SECONDS": "2", "VOUCHER_MIN_MINUTES": "5", "VOUCHER_TTL_DAYS": "30", "SESSION_GRACE_SECONDS": "180",
		}
	}
	existing["NODE_MODE"] = enrollment.NodeMode
	existing["PUBLIC_BASE_URL"] = publicBaseURL
	existing["FRONTEND_BASE_URL"] = frontendBaseURL
	existing["CLOUD_BASE_URL"] = cloudBaseURL
	existing["EDGE_SYNC_TOKEN"] = enrollment.SyncToken
	...
	sort.Strings(keys)
	lines := []string{"# Generated by ClubPay Controller setup. Keep this file private."}
	for _, key := range keys {
		lines = append(lines, key+"="+existing[key])
	}
	configContents := strings.Join(append(lines, ""), "\n")
	...
	if runtime.GOOS == "windows" {
		if reconfigure {
			_, _ = exec.Command("schtasks.exe", "/End", "/TN", "ClubPay Controller Node").CombinedOutput()
		}
