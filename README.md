...
При старте сессии backend создаёт динамический QR типа `session_extend`, привязанный к конкретному `grant_id` и ПК, и передаёт готовый URL в поле `extend_url` команды `start_session`. Agent должен показать этот URL как QR на экране активной сессии; самостоятельно формировать ссылку или токен нельзя. Для такого QR успешная онлайн-оплата или ваучер не создают вторую сессию, а вызывают `POST /core/v1/sessions/{core_session_id}/extend` и добавляют время к текущему окончанию. URL действует только для своей активной сессии и автоматически отключается при её завершении или истечении времени.
