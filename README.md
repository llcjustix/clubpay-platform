...
При старте сессии backend создаёт динамический QR типа `session_extend`, привязанный к корневому `grant_id` и ПК, и передаёт готовый URL в поле `extend_url` команды `start_session`. Agent должен показать этот URL как QR на экране активной сессии; самостоятельно формировать ссылку или токен нельзя. Для каждого успешного продления backend создаёт новый отдельный `grant_id`, но передаёт тот же `core_session_id`: это обеспечивает идемпотентность — повтор одного платежа не добавляет время второй раз. Для такого QR успешная онлайн-оплата или ваучер не создают вторую сессию, а вызывают `POST /core/v1/sessions/{core_session_id}/extend` и добавляют время к текущему окончанию. URL действует только для своей активной сессии и автоматически отключается при её завершении или истечении времени.
