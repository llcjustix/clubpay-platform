...
## Требования пилота из мастер-документа

### Гостевой Wi-Fi и captive-портал

Это инфраструктурная настройка клуба, а не настройка React-приложения:

- гостевой SSID/VLAN должен быть изолирован от игровой и управляющей сетей;
- captive-портал должен открывать публичную QR-страницу Clubpay после подключения к Wi-Fi;
- до прохождения captive-портала разрешаются только домены Clubpay и выбранных платёжных провайдеров (walled garden), с ограничением скорости;
- QR на ПК остаётся источником `pc_id`: Wi-Fi QR может только предложить телефону подключиться к сети, но не может надёжно открыть страницу на всех iOS/Android.

Для работы без облака captive-портал и публичная QR-страница должны указывать на локальный edge-узел клуба. Конкретная конфигурация зависит от роутера/контроллера и выполняется DevOps на площадке.

### Согласие на Telegram-ваучер и наличные

- При передаче номера телефона для Telegram-ваучера клиент должен дать явное согласие. API `POST /api/admin/grants/{grant_id}/end` принимает номер только вместе с `recipient_consent: true`; время и источник согласия сохраняются с ваучером и синхронизируются между edge и cloud.
- Игрок может завершить активный сеанс и на самом ПК: Agent вызывает защищённый `POST /api/core/agent/session/end` с текущими `external_pc_id` и `core_session_id`. Core завершает именно эту сессию, создаёт ваучер на остаток и запускает тот же Telegram-флоу, что и из админки. Если номер ещё не привязан к боту, игрок открывает QR/ссылку бота и нажимает Start — ожидающий ваучер доставляется автоматически.
- Наличная сессия требует код причины: `cash_payment`, `provider_unavailable`, `internet_unavailable` или `terminal_fallback`. В операции и аудите сохраняются менеджер, ПК, сумма, длительность и причина.
...
При старте сессии backend создаёт динамический QR типа `session_extend`, привязанный к корневому `grant_id` и ПК, и передаёт готовый URL в поле `extend_url` команды `start_session`. Agent должен показать этот URL как QR на экране активной сессии; самостоятельно формировать ссылку или токен нельзя. Для каждого успешного продления backend создаёт новый отдельный `grant_id`, но передаёт тот же `core_session_id`: это обеспечивает идемпотентность — повтор одного платежа не добавляет время второй раз. Для такого QR успешная онлайн-оплата или ваучер не создают вторую сессию, а вызывают `POST /core/v1/sessions/{core_session_id}/extend` и добавляют время к текущему окончанию.

После нулевого остатка Agent переводит ПК в `FROZEN`. Динамический QR остаётся доступным только для продления в течение grace-периода; статический QR всё это время закрыт. Длительность задаётся `SESSION_GRACE_SECONDS` (по умолчанию 180 секунд) и передаётся Agent в команде `start_session`. По окончании grace QR и сессия автоматически становятся недействительными.
