...
В master-образ Windows можно установить **только исполняемые файлы Agent и общие настройки
без секретов**. Реальный `CORE_TOKEN` и идентификатор конкретного ПК нельзя сохранять в общем
образе. Git и .NET SDK на игровых ПК не нужны.

### Если пока тестируется один ПК с отдельным образом или локальным SSD
...
или сохранять в документации/общем чате. Не выполняйте эту установку в образе, который затем
будет скопирован на остальные ПК: она запишет клубный секрет и ID первого ПК.
...
В общем `appsettings.json` образа задайте только несекретные шаблоны и не включайте
`appsettings.Local.json` с реальным токеном:
...
После подключения приватного слоя каждого ПК добавьте `Controller:AgentToken` в его
`appsettings.Local.json` **до запуска Agent**. Если слой сбрасывается, токен нужно выдавать
заново при каждой загрузке; состояние сессии и очередь событий должны храниться в постоянном
отдельном хранилище для этого ПК. Так один образ не смешивает сессии: каждый Agent использует
имя своего Windows-клиента и отдельную папку состояния. Если сервер не выдаёт клиентам
уникальные имена, приватный конфиг или постоянную writable-папку, многопользовательский запуск
пока не включаем. Выдача приватного конфига через CPB ещё не реализована и должна быть
проверена на физическом стенде.
